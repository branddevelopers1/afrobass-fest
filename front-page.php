...
<!-- ═══════════════════════════════════════════
     4. SCHEDULE PREVIEW
════════════════════════════════════════════ -->
<section style="position:relative;z-index:2;padding:100px 56px;border-top:1px solid rgba(255,255,255,0.04);background:rgba(255,255,255,0.01);">

  <div style="display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:56px;" class="fest-reveal">
    <div>
      <div class="fest-kicker">August 15, 2026 · Main Stage</div>
      <h2 style="font-family:'Unbounded',sans-serif;font-size:clamp(36px,5vw,64px);font-weight:900;letter-spacing:-1px;color:#fff;text-transform:uppercase;line-height:0.95;margin-top:12px;">Show<br><em style="color:#FF6B1A;font-style:italic;">Schedule</em></h2>
    </div>
    <a href="<?php echo esc_url(home_url('/schedule')); ?>"
       style="font-family:'Space Grotesk',sans-serif;font-size:11px;font-weight:600;letter-spacing:2px;text-transform:uppercase;color:rgba(255,255,255,0.3);text-decoration:none;display:flex;align-items:center;gap:10px;transition:color 0.2s;white-space:nowrap;"
       onmouseover="this.style.color='#fff'" onmouseout="this.style.color='rgba(255,255,255,0.3)'">Full Schedule &rarr;</a>
  </div>

  <div style="max-width:860px;" class="fest-reveal fest-d1">
    <div style="display:grid;grid-template-columns:140px 1fr 160px;padding:0 28px 14px;border-bottom:1px solid rgba(255,255,255,0.05);">
      <span style="font-family:'Space Grotesk',sans-serif;font-size:9px;font-weight:700;letter-spacing:3px;text-transform:uppercase;color:rgba(255,255,255,0.15);">Time</span>
      <span style="font-family:'Space Grotesk',sans-serif;font-size:9px;font-weight:700;letter-spacing:3px;text-transform:uppercase;color:rgba(255,255,255,0.15);">Artist</span>
      <span style="font-family:'Space Grotesk',sans-serif;font-size:9px;font-weight:700;letter-spacing:3px;text-transform:uppercase;color:rgba(255,255,255,0.15);text-align:right;">Role</span>
    </div>
    <?php
    $sched = new WP_Query([
        'post_type'=>'fest_artist','posts_per_page'=>5,
        'meta_key'=>'fest_artist_perf_slot','orderby'=>'meta_value_num','order'=>'ASC',
    ]);
    if ($sched->have_posts()):
      while ($sched->have_posts()): $sched->the_post();
        $role = get_field('fest_artist_role') ?: 'Headliner';
        $time = get_field('fest_artist_perf_time') ?: 'TBA';
        $tba  = get_field('fest_artist_tba');
        $head = in_array($role,['Headliner','Co-Headliner']);
    ?>
      <div style="display:grid;grid-template-columns:140px 1fr 160px;align-items:center;padding:20px 28px;border-bottom:1px solid rgba(255,255,255,0.04);background:<?php echo $head?'rgba(255,255,255,0.015)':'transparent'; ?>;position:relative;">
        <?php if ($head): ?><div style="position:absolute;left:0;top:0;bottom:0;width:2px;background:#FF2D8A;"></div><?php endif; ?>
        <span style="font-family:'Unbounded',sans-serif;font-size:18px;font-weight:900;color:<?php echo $head?'#FF6B1A':'rgba(255,255,255,0.25)'; ?>;"><?php echo esc_html($time); ?></span>
        <div style="display:flex;align-items:center;gap:14px;">
          <?php if (has_post_thumbnail()&&!$tba): ?>
            <div style="width:36px;height:36px;border-radius:50%;overflow:hidden;flex-shrink:0;"><?php the_post_thumbnail('thumbnail',['style'=>'width:100%;height:100%;object-fit:cover;']); ?></div>
          <?php else: ?>
            <div style="width:36px;height:36px;border-radius:50%;background:rgba(255,255,255,0.04);flex-shrink:0;border:1px solid rgba(255,255,255,0.05);"></div>
          <?php endif; ?>
          <span style="font-family:'Unbounded',sans-serif;font-size:<?php echo $head?'18px':'14px'; ?>;font-weight:900;color:<?php echo $tba?'rgba(255,255,255,0.12)':'#fff'; ?>;text-transform:uppercase;"><?php echo $tba?'TBA':get_the_title(); ?></span>
        </div>
        <span style="font-family:'Space Grotesk',sans-serif;font-size:9px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:<?php echo $head?'rgba(255,45,138,0.6)':'rgba(255,255,255,0.18)'; ?>;text-align:right;"><?php echo esc_html($role); ?></span>
      </div>
    <?php endwhile; wp_reset_postdata();
    else:
      $rows=[['TBA','Opening Act','Supporting Act',false],['TBA','Artist TBA','DJ Set',false],['TBA','Artist TBA','Supporting Act',false],['TBA','Headliner','Headliner',true],['TBA','Headliner','Headliner',true]];
      foreach ($rows as $r): ?>
      <div style="display:grid;grid-template-columns:140px 1fr 160px;align-items:center;padding:20px 28px;border-bottom:1px solid rgba(255,255,255,0.04);background:<?php echo $r[3]?'rgba(255,255,255,0.015)':'transparent'; ?>;position:relative;">
        <?php if ($r[3]): ?><div style="position:absolute;left:0;top:0;bottom:0;width:2px;background:#FF2D8A;"></div><?php endif; ?>
        <span style="font-family:'Unbounded',sans-serif;font-size:18px;font-weight:900;color:<?php echo $r[3]?'#FF6B1A':'rgba(255,255,255,0.2)'; ?>;"><?php echo esc_html($r[0]); ?></span>
        <div style="display:flex;align-items:center;gap:14px;">
          <div style="width:36px;height:36px;border-radius:50%;background:rgba(255,255,255,0.03);flex-shrink:0;border:1px solid rgba(255,255,255,0.05);"></div>
          <span style="font-family:'Unbounded',sans-serif;font-size:<?php echo $r[3]?'18px':'14px'; ?>;font-weight:900;color:rgba(255,255,255,0.12);text-transform:uppercase;"><?php echo esc_html($r[1]); ?></span>
        </div>
        <span style="font-family:'Space Grotesk',sans-serif;font-size:9px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:<?php echo $r[3]?'rgba(255,45,138,0.4)':'rgba(255,255,255,0.12)'; ?>;text-align:right;"><?php echo esc_html($r[2]); ?></span>
      </div>
    <?php endforeach; endif; ?>
    <div style="padding:14px 28px;display:flex;justify-content:space-between;">
      <span style="font-family:'Space Grotesk',sans-serif;font-size:9px;font-weight:600;letter-spacing:2px;text-transform:uppercase;color:rgba(255,255,255,0.12);">Rebel Entertainment Complex · Doors Open TBA</span>
      <span style="font-family:'Space Grotesk',sans-serif;font-size:9px;font-weight:600;letter-spacing:2px;text-transform:uppercase;color:rgba(255,255,255,0.12);">19+</span>
    </div>
  </div>
...
<!-- ═══════════════════════════════════════════
     6. VENUE
════════════════════════════════════════════ -->
<section style="position:relative;z-index:2;padding:100px 56px;border-top:1px solid rgba(255,255,255,0.04);display:grid;grid-template-columns:1fr 1fr;gap:80px;align-items:center;">

  <div class="fest-reveal">
    <div class="fest-kicker">The Venue</div>
    <h2 style="font-family:'Unbounded',sans-serif;font-size:clamp(30px,4vw,52px);font-weight:900;letter-spacing:-1px;color:#fff;text-transform:uppercase;line-height:0.95;margin-top:12px;margin-bottom:28px;">Rebel<br>Entertainment<br><em style="color:#FF6B1A;font-style:italic;">Complex</em></h2>
    <p style="font-size:15px;font-weight:300;color:rgba(255,255,255,0.4);line-height:1.85;margin-bottom:32px;">One of Toronto's most iconic live music and event venues, right on the waterfront. Built for massive productions, incredible sound, and unforgettable nights. The perfect stage for Afrobass Music Festival's first edition.</p>
    <div style="display:flex;flex-direction:column;">
      <?php foreach([['Address','11 Polson St, Toronto, ON'],['Capacity','3,000+ Attendees'],['Date','Saturday, August 15, 2026'],['Age','19+ Event']] as $vf): ?>
        <div style="display:flex;gap:20px;padding:13px 0;border-bottom:1px solid rgba(255,255,255,0.04);">
          <span style="font-family:'Space Grotesk',sans-serif;font-size:9px;font-weight:700;letter-spacing:3px;text-transform:uppercase;color:rgba(255,255,255,0.18);min-width:72px;padding-top:2px;"><?php echo esc_html($vf[0]); ?></span>
          <span style="font-size:13px;color:rgba(255,255,255,0.65);"><?php echo esc_html($vf[1]); ?></span>
        </div>
      <?php endforeach; ?>
    </div>
  </div>

  <div class="fest-reveal fest-d2" style="position:relative;height:460px;background:#060606;overflow:hidden;border:1px solid rgba(255,255,255,0.04);">
    <div style="position:absolute;inset:0;background:radial-gradient(ellipse 80% 80% at 50% 50%,rgba(255,107,26,0.1) 0%,transparent 65%),repeating-linear-gradient(0deg,transparent,transparent 39px,rgba(255,255,255,0.025) 39px,rgba(255,255,255,0.025) 40px),repeating-linear-gradient(90deg,transparent,transparent 39px,rgba(255,255,255,0.025) 39px,rgba(255,255,255,0.025) 40px);"></div>
    <div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-60%);">
      <div style="width:52px;height:52px;border-radius:50%;background:#FF6B1A;display:flex;align-items:center;justify-content:center;box-shadow:0 0 48px rgba(255,107,26,0.6);">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3" fill="#fff" stroke="none"/></svg>
      </div>
      <div style="width:2px;height:28px;background:linear-gradient(to bottom,#FF6B1A,transparent);margin:0 auto;"></div>
    </div>
    <div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-60%);width:110px;height:110px;border-radius:50%;border:1px solid rgba(255,107,26,0.18);animation:fPulseRing 2s ease-in-out infinite;"></div>
    <div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-60%);width:170px;height:170px;border-radius:50%;border:1px solid rgba(255,107,26,0.09);animation:fPulseRing 2s 0.6s ease-in-out infinite;"></div>
    <div style="position:absolute;bottom:28px;left:28px;right:28px;">
      <div style="font-family:'Unbounded',sans-serif;font-size:10px;font-weight:700;letter-spacing:3px;text-transform:uppercase;color:rgba(255,255,255,0.35);">Rebel Entertainment Complex</div>
      <div style="font-size:11px;color:rgba(255,255,255,0.18);margin-top:4px;">11 Polson St · Toronto Waterfront</div>
    </div>
    <style>@keyframes fPulseRing{0%,100%{opacity:1;transform:translate(-50%,-60%) scale(1);}50%{opacity:0.3;transform:translate(-50%,-60%) scale(1.06);}}</style>
  </div>
...
<!-- ═══════════════════════════════════════════
     7. SPONSORS
════════════════════════════════════════════ -->
<section style="position:relative;z-index:2;padding:80px 56px;border-top:1px solid rgba(255,255,255,0.04);background:rgba(255,255,255,0.01);">

  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:48px;" class="fest-reveal">
    <div class="fest-kicker" style="margin-bottom:0;">Official Partners</div>
    <a href="<?php echo esc_url(home_url('/sponsors')); ?>"
       style="font-family:'Space Grotesk',sans-serif;font-size:11px;font-weight:600;letter-spacing:2px;text-transform:uppercase;color:rgba(255,255,255,0.2);text-decoration:none;transition:color 0.2s;"
       onmouseover="this.style.color='#fff'" onmouseout="this.style.color='rgba(255,255,255,0.2)'">Become a Sponsor &rarr;</a>
  </div>

  <?php if ($sponsors->have_posts()): ?>
    <div style="display:flex;flex-wrap:wrap;gap:2px;background:rgba(255,255,255,0.04);" class="fest-reveal fest-d1">
      <?php while ($sponsors->have_posts()): $sponsors->the_post();
        if (get_field('fest_sponsor_visible') === false) continue;
        $surl = get_field('fest_sponsor_url') ?: '#';
        $stag = get_field('fest_sponsor_tagline') ?: '';
      ?>
        <a href="<?php echo esc_url($surl); ?>" target="_blank" rel="noopener"
           style="flex:1;min-width:180px;background:#080808;padding:32px 24px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;text-decoration:none;transition:background 0.2s;"
           onmouseover="this.style.background='#0d0d0d'" onmouseout="this.style.background='#080808'">
          <?php if (has_post_thumbnail()): ?>
            <?php the_post_thumbnail('medium',['style'=>'max-height:44px;width:auto;filter:brightness(0) invert(1);opacity:0.35;','class'=>'sponsor-logo']); ?>
          <?php else: ?>
            <div style="font-family:'Unbounded',sans-serif;font-size:13px;font-weight:700;letter-spacing:1px;color:rgba(255,255,255,0.2);"><?php the_title(); ?></div>
          <?php endif; ?>
          <?php if ($stag): ?><div style="font-family:'Space Grotesk',sans-serif;font-size:9px;font-weight:600;letter-spacing:2px;text-transform:uppercase;color:rgba(255,255,255,0.12);"><?php echo esc_html($stag); ?></div><?php endif; ?>
        </a>
      <?php endwhile; wp_reset_postdata(); ?>
    </div>
  <?php else: ?>
    <div class="fest-reveal fest-d1" style="background:#080808;border:1px solid rgba(255,255,255,0.04);padding:56px;text-align:center;">
      <div style="font-family:'Unbounded',sans-serif;font-size:12px;font-weight:700;letter-spacing:3px;text-transform:uppercase;color:rgba(255,255,255,0.1);margin-bottom:12px;">Sponsorship Opportunities Available</div>
      <p style="font-size:13px;font-weight:300;color:rgba(255,255,255,0.2);margin-bottom:24px;">Place your brand in front of 3,000+ Afrobeats fans in Toronto. Platinum, Gold, Silver, Bronze &amp; In-Kind packages available.</p>
      <a href="<?php echo esc_url(home_url('/sponsors')); ?>"
         style="display:inline-block;font-family:'Space Grotesk',sans-serif;font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#FF6B1A;text-decoration:none;border:1px solid rgba(255,107,26,0.3);padding:14px 32px;border-radius:2px;transition:border-color 0.2s;"
         onmouseover="this.style.borderColor='#FF6B1A'" onmouseout="this.style.borderColor='rgba(255,107,26,0.3)'">View Packages &rarr;</a>
    </div>
  <?php endif; ?>
